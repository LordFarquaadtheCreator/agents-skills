...
1. **One-time setup**: Call `set_embedding_config` with the embedding endpoint (e.g. LM Studio at `http://localhost:1234`, model `text-embedding-embeddinggemma-300m-qat`). This persists on disk. This most likely is done, and should be suspected if there are errors. 

2. **Initialize resume**: Call `init_resume` with full structured resume data. This stores the resume and builds a vector store embedding every bullet point. Re-calling this overwrites everything. This most likely is done, and should be suspected if there are errors. 

3. **Ask user**: Auto or manual?
   - **Auto**: Call `generate_resume` with `mode="auto"`, `query="<job description>"`, `template="<template>"`. MCP searches vector store, selects most relevant content, enforces one page, outputs PDF.
   - **Manual**: Call `search_resume` with the job description. Review results with user. Agent (you) tailors content (selects/reorders bullets, drops irrelevant items). Call `generate_resume` with `mode="manual"`, `data=<tailored ResumeData>`, `template="<template>"`.
