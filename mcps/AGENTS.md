...
| [`pawchive-mcp`](pawchive-mcp/) | Read-only MCP wrapping pawchive.pw's public API. 16 tools covering creators, posts, comments, revisions, flag checks, hash search, app version, and media download. `download_post_media` re-encodes CDN images, container-rasterizes PDFs, and writes base64 sanitized media to temp files. API responses use a temporary file cache with a one-day TTL and per-call bypass. Go + modelcontextprotocol/go-sdk. No auth, no mutations. |
